...
const removeTask = async (req: Request, res: Response) => {
  try {
    ...
    //@ts-ignore
    if (req.user.id !== task.userId && req.user.role === "basic") { // authenticate user and only allow him to delete his own content
      res
        .status(400)
        .json({ message: "You are not authorized to delete this task." });
      return;
    }
    if(req.user.role ==='admin'){ // explicitly allow only admins to delete other users' content
